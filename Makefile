...
valgrind: build
	ulimit -n 65536 && valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --verbose ./$(BUILD_DIR)/$(EXECUTABLE_NAME)

valgrind-definite: build
	ulimit -n 65536 && valgrind --leak-check=full --errors-for-leak-kinds=definite ./$(BUILD_DIR)/$(EXECUTABLE_NAME)
